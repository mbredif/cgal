...
/// @{
    /// \brief executes a function on all values of the assiociative container `c`, calling `transform(v)` on each value `v` in `c`.
    ...
    /// \tparam Transform a model of `Callable` with a reference to the `mapped_type` of `Container` as its argument type
    ...
    /// the values returned by `transform(v1, v2)` on all values `v1` and `v2` that share a key `k`.
    ...
    ///
    /// \tparam Container1 a model of an `AssociativeContainer`, such as `std::map`
    /// \tparam Container2 a model of an `AssociativeContainer`, such as `std::map`
    /// \tparam V the return type of the function
    /// \tparam Transform a model of `Callable` with references to the `mapped_type`s of `Container`s as its argument types
    ...
    /// \brief repeatedly joins two associative containers `c1` and `c2` by key and aggregate `v=reduce(v, u)` with `u=transform(v1,v2)` and
    ///        values `v1,v2` sharing a key in the containers `c1,c2`, starting with `v=init`. Repeated joins occur until convergence, meaning that
