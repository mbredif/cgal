...
public:
    ...
    /// constructor
    Tile_triangulation(Id id, Traits t)
        : traits(t),
          id_(id),
    ...
    bool are_vertices_equal(Vertex_const_handle v, const Tile_triangulation& t, Vertex_const_handle tv) const
    {
        return traits.are_vertices_equal(dt_, v, t.dt_, tv);
    }

    bool are_facets_equal(Facet_const_handle f, const Tile_triangulation& t, Facet_const_handle tf) const
    {
        return traits.are_facets_equal(dt_, f, t.dt_, tf);
    }

    bool are_cells_equal(Cell_const_handle c, const Tile_triangulation& t, Cell_const_handle tc) const
    {
    ...
    Vertex_const_handle relocate_vertex(const Tile_triangulation& t, Vertex_const_handle v, Vertex_handle hint = Vertex_handle()) const
    {
        if(t.vertex_is_infinite(v)) return infinite_vertex();
        return locate_vertex(t.point(v), hint);
    }

    Facet_const_handle relocate_facet(const Tile_triangulation& t, Facet_const_handle f) const
    {
    ...
    Cell_const_handle relocate_cell(const Tile_triangulation& t, Cell_const_handle c) const
    {
